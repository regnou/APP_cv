...
async function edit(COLLECTION_PATH, entity, caller = '') {
	...
	try {
		...
		console.log(caller + ' ⚡✅ EDIT > Edited doc-with-ID: [', COLLECTION_PATH, '] > ', entity.id);
		return true;
	} catch (e) {
...
async function get(COLLECTION_PATH, docId, caller = '') {
	...
	try {
		...
		if (_docSnap.exists()) {
			...
			console.warn(caller + ' ⚡⚠️ GET > No such document! :: [', COLLECTION_PATH, '/', docId, ']');
			return null;
		} // 1
	} catch (e) {
...
async function getByFilter(COLLECTION_PATH, filterPredicat, filterValue, caller = '') {
	...
	try {
		...
		console.error(caller + ' ⚡❌ [getByFilter]  - details > COLLECTION_PATH: ', COLLECTION_PATH);
